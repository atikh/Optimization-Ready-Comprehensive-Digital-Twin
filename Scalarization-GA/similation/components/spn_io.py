...
def write_to_event_log(simulation_time, token_ids, event, transition, spn):
    ...
    try:
        ...
        dims_source = getattr(spn, "executable_dimensions", [])
        ordered_dims = [dim for dim in dims_source if not is_time_dimension(dim)]

        dimension_values = [round(spn.get_dimension_value(dim), 2) for dim in ordered_dims]
        ...
        with open(file_path, "a", newline="") as event_log:
            ...
            if write_header:
                header = ["Time_Stamp", "ID"] + ordered_dims + ["Event"]
