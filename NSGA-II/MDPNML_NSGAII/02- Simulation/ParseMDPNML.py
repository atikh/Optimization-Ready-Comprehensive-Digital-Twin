...
def parse_mdpnml_to_spn(mdpnml, verbose=True):
    if verbose:
    ...
    for element in root.findall(".//mdpnml:transition", ns):
        ...
        # Keep the MDPNML identity and original distribution metadata so that
        # an optimizer can update the parsed transition without searching by label.
        transition.mdpnml_id = transition_id
        # Preserve the exact label read from MDPNML.  Different MDPySPN
        # versions expose a Transition label through different attributes
        # (for example ``label`` rather than ``name``), so optimization and
        # KPI extraction must not depend on the library-specific attribute.
        transition.mdpnml_name = str(name)
        transition.mdpnml_distribution_type = dist_type
        transition.mdpnml_distribution_params = dict(dist_params)
    ...
    # Preserve MDPNML identifiers for optimization, diagnostics, and export.
    spn_model.places_by_id = places_dict
    spn_model.transitions_by_id = transitions_dict
    spn_model.mdpnml_path = str(mdpnml)
